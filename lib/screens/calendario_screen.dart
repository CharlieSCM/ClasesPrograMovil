...
//CREAR FUNCIONES (FUTURO Y UNA CON RETORNO DE LISTA PARA ASIGNAR A MAPA ITERABLE)
  ...
  Future getDates() async {
    var res = await agendaDB!.GETALLTASK();
    var dates = [];
    Map<DateTime, List<Event>> aux = {};
    List<Event> events = [];
    res.forEach((element) {
      if (!dates.contains(element.fecExpiracion)) {
        dates.add(element.fecExpiracion);
      }
    });
    for (int i = 0; i < dates.length; i++) {
      for (int j = 0; j < res.length; j++) {
        if (dates[i] == res[j].fecExpiracion) {
